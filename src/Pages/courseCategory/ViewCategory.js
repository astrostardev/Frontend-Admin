...
function Category() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsloading(true)
                const response = await fetch(`${process.env.REACT_APP_URL}/api/v1/course_category/show`, {
                    method: 'GET',
                    headers: {
                ...
                if (!response.ok) {
                    ...
                    const data = await response.json();
                     console.log(data);
                    setCourseCategory(data.courseCategories)
    ...
    const columns = [
       ...
       
        {
            field: 'details',
            headerName: 'Details',
            renderCell: (params) => <Button style={{backgroundColor:"rgb(54, 75, 182)",color:"#fff"}} onClick={() => navigate(`/edit_course_categories/${params?.row._id}`)} >
                View
            </Button>,
            width: 100,
            ...
            <main id="admin-astro">
                <section className="astro-head">
                    ...
                    <div style={{display:"flex", gap:"10px"}}>
                        <Link to="/add_course_category" className="addAstroLink">Add Category</Link>
